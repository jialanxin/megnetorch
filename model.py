...
def ff(input_dim):
    return torch.nn.Sequential(torch.nn.Linear(input_dim, 64), torch.nn.RReLU(), torch.nn.Dropout(), torch.nn.Linear(64, 32), torch.nn.Dropout(), torch.nn.RReLU())


def fff(input_dim):
    return torch.nn.Sequential(torch.nn.Linear(input_dim, 64), torch.nn.RReLU(), torch.nn.Dropout(), torch.nn.Linear(64, 64), torch.nn.RReLU(), torch.nn.Dropout(), torch.nn.Linear(64, 32), torch.nn.RReLU())


def ff_output(input_dim, output_dim):
    return torch.nn.Sequential(torch.nn.Linear(input_dim, 128), torch.nn.RReLU(), torch.nn.Dropout(), torch.nn.Linear(128, 64), torch.nn.RReLU(), torch.nn.Dropout(), torch.nn.Linear(64, output_dim))
...
class MegNetLayer(torch.nn.Module):
    ...
    def forward(self, bonds, bond_atom_1, bond_atom_2, atoms, state):
        ...
        bonds = torch.cat((atom_1_to_bonds, atom_2_to_bonds, bonds, state.repeat(
            (sum_of_num_bonds, 1))), dim=1)  # (sum_of_num_bonds,atom_info*2+bond_info+state_info)
        bonds = self.phi_e(bonds)  # (sum_of_num_bonds,bond_info)

        # (sum_of_num_atoms,bond_info) here because bond_info and atom_info are both 32
        bonds_to_atoms = torch.zeros_like(atoms)
        sum_of_num_atoms = atoms.shape[0]
        count_bonds_to_atoms = torch.zeros_like(
            atoms)  # (sum_of_num_atoms,bond_info)
        # for ix_bond in range(sum_of_num_bonds):
        ...
        bonds_to_atoms = bonds_to_atoms.scatter_add(
            dim=0, index=bond_atom_1, src=bonds)
        count_bonds_to_atoms = count_bonds_to_atoms.scatter_add(
            dim=0, index=bond_atom_1, src=torch.ones_like(bonds))
        # (sum_of_num_atoms,bond_info)
        bonds_to_atoms = bonds_to_atoms/count_bonds_to_atoms
        # (sum_of_num_atoms,bond_info+atom_info+state_info)
        atoms = torch.cat(
            (bonds_to_atoms, atoms, state.repeat((sum_of_num_atoms, 1))), dim=1)
        atoms = self.phi_v(atoms)  # (sum_of_num_atoms,atom_info)

        bonds_to_state = torch.mean(
            bonds, dim=0, keepdim=True)  # (1,bond_info)
        atoms_to_state = torch.mean(
            atoms, dim=0, keepdim=True)  # (1,atom_info)
        # (1,bond_info+atom_info+state_info)
...
class FirstMegnetBlock(torch.nn.Module):
    ...
    def forward(self, bonds, bond_atom_1, bond_atom_2, atoms, state):
        ...
        residual_bonds, residual_atoms, residual_state = self.megnetlayer(
            residual_bonds, bond_atom_1, bond_atom_2, residual_atoms, residual_state)
...
class MegNet(torch.nn.Module):
    ...
    def forward(self, atoms, state, bonds, bond_atom_1, bond_atom_2, batch_mark_for_atoms, batch_mark_for_bonds):
        ...
        bonds, atoms, state = self.firstblock(
            bonds, bond_atom_1, bond_atom_2, atoms, state)
        for block in self.fullblocks:
            bonds, atoms, state = block(
                bonds, bond_atom_1, bond_atom_2, atoms, state)
        ...
        # (batch_size, bond_info+atom_info+state_info)
        gather_all = torch.cat(
            (bonds, atoms, state.repeat(batch_size, 1)), dim=1)
